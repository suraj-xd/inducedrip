...
import { ShiningText } from "./21st/shining-text";

export function AiTryonNotifications() {
  const { generations, removeGeneration, cancelGeneration } =
    useAiTryonStore();
  const [selectedResult, setSelectedResult] = useState<AiGeneration | null>(
    null
  );
    ...
    <>
      <div className="fixed bottom-4 right-4 z-50 space-y-2 max-w-sm">
        {generations.map((generation) => (
          <div
            key={generation.id}
            className="bg-white/80 border border-gray-200 rounded-2xl backdrop-blur-xl shadow-lg p-4 min-w-[300px] animate-in slide-in-from-right-full"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <ShiningText text={getStatusText(generation.status)} />
                  ) : (
                    <>
